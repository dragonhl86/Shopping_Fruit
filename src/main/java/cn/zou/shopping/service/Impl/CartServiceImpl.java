...
public class CartServiceImpl implements CartService {
    ...
    public Msg addCartItem(long itemId, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (cartItem == null) {
            ...
            Msg msg = fruitService.getFruitByPrimary(itemId);
            if (msg.getCode() == 100) {
                Fruit fruit = (Fruit) msg.getExtend().get("fruit");
                cartItem.setQuantity(1);
                cartItem.setB_id(fruit.getbId());
                cartItem.setId(fruit.getbId());
                cartItem.setC_id(fruit.getC_id());
                cartItem.setFruit_image(fruit.getFruitImage());
                cartItem.setFruit_name(fruit.getFruitName());
                cartItem.setPrice(fruit.getPrice());
                cartItem.setSource(fruit.getSource());
            }
            //添加到购物车列表
